...
if (isset($_POST['action'])) 
{
	if ($_POST["action"] == "fetch")
	{
		...
		foreach ($result as $row) 
		{
			...
			$sub_array[] = '<a href="edit_teacher.php?action='.base64_encode($row['teacher_id']).'"><button type="button" name="edit_teacher" class="btn btn-primary btn-sm edit_teacher" id="'.$row['teacher_id'].'">Edit</button></a>';
	...
	if ($_POST['action'] == "Add" || $_POST['action'] == "Edit")
	{
		...
		if($_FILES["teacher_image"]["name"] != '')
		{
			...
			{
				$error_teacher_image = 'Invalid Image Extension';
				$error++;
			}
			else
			{
		...
		if($_POST["action"] == "Add")
		{
			...
			{
				$error_teacher_emailid = "Email id is required";
				$error++;
			}
			else
			{
				...
				{
					$error_teacher_emailid = "Invalid Email Formate";
					$error++;
				}
				else
				{
			...
			{
				$error_teacher_grade_id = "Grade Id is Required";
				$error++;
			}
			else
			{
			...
			{
				$error_teacher_doj = "Date of join is Required";
				$error++;
			}
			else
			{
