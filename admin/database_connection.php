...
{
	$query = "
		SELECT student_name FROM tbl_student WHERE student_id = '$student_id'
	";
...
{
	$query = "
		SELECT tbl_teacher.teacher_name FROM tbl_student
		INNER JOIN tbl_grade
		ON tbl_grade.grade_id = tbl_student.student_grade_id
...
{
	$query = "
			SELECT grade_name FROM tbl_grade WHERE grade_id = '$grade_id'
	";
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
