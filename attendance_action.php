...
if(isset($_POST["action"]))
{
	...
	if($_POST["action"] == "Add")
	{
		...
		{
			$error_attendance_date = "Date is Required";
			$error++;
		...
		else
		{
			...
			if($statement->rowCount() > 0 )
			{
				$output = array(
					'error' => true,
					'error_attendance_date' => 'Attendance Data Already Exist on this Date'
				);
			}
			else
			{
	...
	if($_POST["action"] == "index_fetch")
	{
		...
		foreach ($result as $row)
		{
			...
			$sub_array[] = $row["student_roll_number"];
			$sub_array[] = $row["grade_name"];
			if(get_attendance_percentage($connect, $row["student_id"]) <= 33)
			{
				$sub_array[] = '<div class="progress">
				<div class="progress-bar bg-danger" style="width:'.get_attendance_percentage($connect, $row["student_id"]).'%"><div class="text-white text-center">'.get_attendance_percentage($connect, $row["student_id"]).'%</div></div>
				</div>';
			}
			elseif(get_attendance_percentage($connect, $row["student_id"]) > 33 && get_attendance_percentage($connect, $row["student_id"]) <= 70)
			{
				$sub_array[] = '<div class="progress">
				<div class="progress-bar bg-warning" style="width:'.get_attendance_percentage($connect, $row["student_id"]).'%"><div class="text-white text-center">'.get_attendance_percentage($connect, $row["student_id"]).'%</div></div>
				</div>';
			}
			else{

				$sub_array[] = '<div class="progress">
				<div class="progress-bar bg-success" style="width:'.get_attendance_percentage($connect, $row["student_id"]).'%"><div class="text-white text-center">'.get_attendance_percentage($connect, $row["student_id"]).'%</div></div>
				</div>';
